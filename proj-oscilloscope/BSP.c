 ...
 *	Vers�o: 2.0
 *	Foi implementada um adapta��o da fun��o BSP_LCD_DrawBitmap para trabalhar com 4 bits (1byte por pixel)
 *	Foi implementada uma fun��o para impress�o de Bitmap no formato de 24 bits
 */
...
#include "tm4c123gh6pmX.h"	//Leandro (12/02/2019) - alterei o nome para evitar conflito com CMSIS
...
//Leandro (12/02/2019) - adapta��o fa fun��o BSP_LCD_DrawBitmap para trabalhar com 4 bits (1 byte por pixel)
...
// displayed on the screen (encoded in reverse order, which is
// standard for bitmap files).  An array can be created in this
// format from a 4-bit-per-pixel .bmp file using the associated
// converter program.
// (x,y) is the screen location of the lower left corner of BMP image
// Requires (11 + w*h) bytes of transmission (assuming image fully on screen)
// Input: x     horizontal position of the bottom left corner of the image, columns from the left edge
//        y     vertical position of the bottom left corner of the image, rows from the top edge
//        image pointer to a 8-bit color BMP image
//        w     number of pixels wide
//        h     number of pixels tall
// Output: none
// Must be less than or equal to 128 pixels wide by 128 pixels high
void BSP_LCD_DrawBitmap4Bits(int16_t x, int16_t y, const unsigned char *image, int16_t w, int16_t h)
{
	uint16_t pixel;
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
			...
			switch(image[i])
			{
				case	LCD4_Black:					pixel = LCD_Black;				break;
				case	LCD4_Dark_Blue:			pixel = LCD_Dark_Blue;		break;
				case	LCD4_Green:					pixel = LCD_Green;				break;
				case	LCD4_Teal:					pixel = LCD_Teal;					break;
				case	LCD4_Dark_Red:			pixel = LCD_Dark_Red;			break;
				case	LCD4_Violet:				pixel = LCD_Violet;				break;
				case	LCD4_Dark_Yellow:		pixel = LCD_Dark_Yellow; 	break;
				case	LCD4_Gray50:				pixel = LCD_Gray50;				break;
				case	LCD4_Gray25:				pixel = LCD_Gray25;				break;
				case	LCD4_Blue:					pixel = LCD_Blue;					break;
				case	LCD4_Bright_Green:	pixel = LCD_Bright_Green; break;
				case	LCD4_Turquoise:			pixel = LCD_Turquoise;	 	break;
				case	LCD4_Red:						pixel = LCD_Red;					break;
				case	LCD4_Pink:					pixel = LCD_Pink;					break;
				case	LCD4_Yellow:				pixel = LCD_Yellow;				break;
				case	LCD4_White:					pixel = LCD_White;				break;
				default: 									pixel =	LCD_Black;				break;
			}
			                                  // send the top 8 bits
      writedata((uint8_t)(pixel >> 8));
                                        // send the bottom 8 bits
      writedata((uint8_t)pixel);	
...
//Leandro (03/03/2019) - fun��o para impress�o de Bitmap no formato 24 bits
// (x,y) is the screen location of the lower left corner of BMP image
// Requires (11 + w*h) bytes of transmission (assuming image fully on screen)
// Input: x     horizontal position of the bottom left corner of the image, columns from the left edge
//        y     vertical position of the bottom left corner of the image, rows from the top edge
